...
def save_qr_image(matrix, filepath, scale=10, border=4, label=None):
    """
    Saves a QR matrix as a pixel-perfect PNG image, optionally adding a text label.
    
    matrix: 2D list of 0s and 1s (where 1 is dark, 0 is light)
    filepath: destination file path
    scale: pixels per module
    border: quiet zone border size in modules
    label: text label to draw at the bottom of the image
    """
    S = len(matrix)
    new_size = S + 2 * border
    ...
    if label:
        ...
        font_size = max(12, int(scale * 1.5))
        extra_height = int(font_size * 2.0)
        
        width, height = img.size
        # Create a new white image with the extra height for the label
        labeled_img = Image.new('1', (width, height + extra_height), 1)
        ...
        y = height + (extra_height - text_height) // 2 - 2
...
def main():
    ...
    parser.add_argument('--label', type=str, nargs='?', const='auto', default=None,
                        help='Draw a human-readable text label at the bottom of the QR code. '
                             'Defaults to the base filename if specified without a value.')
    ...
    # Delete old generated images to prevent stale chunk files
    import glob
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        if args.label is not None:
            if args.label == 'auto':
                ...
                if total > 1:
                    label_text = f"{args.label} ({idx}/{total})"
                else:
                    label_text = args.label
                    
        save_qr_image(matrix, filepath, scale=args.scale, border=args.border, label=label_text)
